...
function inserirPontuacoes(req, res) {
    const {dificuldade, pontos, erros, acertos } = req.body; // Dados da pontuação recebidos no corpo da solicitação
    var idUsuario = req.params.idUsuario;
    // Chamando a função do modelo para inserir a nova pontuação no banco de dados
    pontuacoesModel.inserirPontuacoes(idUsuario, dificuldade, pontos, erros, acertos, (error) => {
        if (error) {
